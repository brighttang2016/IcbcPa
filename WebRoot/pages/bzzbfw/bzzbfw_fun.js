...
function showDetail(grid){
	...
	if(records.length == 0){
		...
		recordCol.add("unit",record.get("unit"));
		recordCol.add("point",record.get("point"));
...
function getEditTbCol(gridId){
	var bzcpCombo = Ext.create('bzcpCombo');
	var pubColumns = [{header:'指标序号',dataIndex:'seq_num',width:220,editor:bzcpCombo},
	                   {header:'指标名称',dataIndex:'zb_name',width:160},
	                   {header:'mova标准分值',dataIndex:'point',width:100},
...
//								iconCls:'Add',
	...
	bzcpCombo.on('expand',function(){
		bzcpCombo.getStore().reload();
	});
	bzcpCombo.on('select',function(combo,records,eOpts){
...
//					document.location = "downLoad.html?txCode=30062";//会导致【MessagePush.onPageLoad(userId);】已注册的后台监听失效
	...
	var winAdd = getWinAdd("添加机构标准产品指标范围",zhxfGrid);
	winAdd.show();
}
