	...
	launch:function(){
		...
		checkForm = Ext.create('Ext.form.Panel',{
			...
			buttons:[{
				text:'查询',
				handler:function(){
					comm_query('gridStoreId','checkForm','20000004');
				}
			},{
				text:'清空',
				handler:function(){
//					checkForm.getForm().reset();
		...
		var myColumns = [ 
            ...
            {header:'柜员履职考核总包',dataIndex:'zb_gylz',width:150,align:'right'},
            {header:'客户经理履职总包',dataIndex:'zb_khjllz',width:150,align:'right'},
            {header:'全员营销奖励总包',dataIndex:'zb_qyyx',width:150,align:'right'}
        ];
...
//			}),
			tbar:[{
				...
				iconCls:'Folderup',
				handler:function(){
					fileUpload("20000003","总包导入(支行考核)");
				}
			},{
				text:'下载批量模版',
				iconCls:'Packagedown',
				handler:function(){
					Ext.getDom('downloadIframe').src = "downLoad.html?tx_code=20000003";
				}
			}],
