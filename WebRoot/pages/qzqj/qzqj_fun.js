...
//					saveAll(zhjfStore,txCode,winAdd);
					...
					new comm().saveAll("gridStoreId","zhjfStoreId",keyArray,txCode,winAdd,'assManage.html');
				}
			}]
		},{
			text:'批量导入',
...
//					document.location = "downLoad.html?txCode=30062";//会导致【MessagePush.onPageLoad(userId);】已注册的后台监听失效
					Ext.getDom('downloadIframe').src = "downLoad.html?tx_code=30084";
				}
			}]
		}]
	});
	var winAdd = getWinAdd("添加不参与考核人员",qzqjGrid);
	winAdd.show();
}
