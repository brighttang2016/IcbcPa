...
//		var orgInfoQuery = orgCmp.getOrgInfoQuery(orgTreePanel);
		...
		checkForm = Ext.create('Ext.form.Panel',{
			...
			buttons:[{
				text:'查询',
				handler:function(){
					comm_query('gridStoreId','checkForm','20000014');
				}
			},{
				text:'清空',
				handler:function(){
...
//		    {header:'网点编号',dataIndex:'orgid',width:110,align:'left'},
            {header:'网点名称名称',dataIndex:'orgname',width:170,align:'left'},
//            {header:'部门编号',dataIndex:'orgname',width:170,align:'left'},
            {header:'部门名称',dataIndex:'depname',width:170,align:'left'},
            ...
            {header:'手动分配绩效',dataIndex:'jx',width:120,align:'right'}
           ];
...
//			}),
			tbar:[{
				...
				iconCls:'Folderup',
				handler:function(){
					fileUpload("20000013","网点手动分配绩效导入");
				}
			},'-',{
				text:'下载批量模版',
				iconCls:'Packagedown',
				handler:function(){
					Ext.getDom('downloadIframe').src = "downLoad.html?tx_code=20000013";
				}
			},'-',{
				text:'查询可分配总包',
				iconCls:'Databaseyellow',
				handler:function(){
					showAdd();
				}
			}],
