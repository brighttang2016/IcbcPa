	...
	launch:function(){
		...
		checkForm = Ext.create('Ext.form.Panel',{
			...
			buttons:[{
				text:'查询',
				handler:function(){
					comm_query('gridStoreId','checkForm','20000002');
				}
			},{
				text:'清空',
				handler:function(){
//					checkForm.getForm().reset();
		...
		var myColumns = [ 
		    ...
		    {header:'分支行号',dataIndex:'orgid',width:110,align:'left'},
            {header:'分支行名称',dataIndex:'orgname',width:170,align:'left'},
            {header:'人力资源编码',dataIndex:'userid',width:120,align:'left'},
            {header:'姓名',dataIndex:'name',width:120,align:'left'},
            {header:'考核周期',dataIndex:'zq',width:80,align:'left'},
            {header:'履职考核得分',dataIndex:'pt_gwlz',width:150,align:'left'},
            {header:'网点挂钩考核得分',dataIndex:'pt_wdgg',width:150,align:'left'},
            {header:'全员营销奖励得分',dataIndex:'pt_qyyx',width:150,align:'left'}
         ];
...
//			}),
			tbar:[{
				...
				iconCls:'Folderup',
				handler:function(){
					fileUpload("20000001","个人业绩导入");
				}
			},{
				text:'下载批量模版',
				iconCls:'Packagedown',
				handler:function(){
					Ext.getDom('downloadIframe').src = "downLoad.html?tx_code=20000001";
