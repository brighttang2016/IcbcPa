...
//		    Ext.create('Ext.grid.RowNumberer',{text:'行号',width:40,align:'center'}),//行号
		    {header:'人力资源编码',dataIndex:'userId',width:110,sortable:false,align:'center',locked:true},
			...
			{header:'人员类别',dataIndex:'jbName',width:100,sortable:true,align:'center',locked:true},
			{header:'大机业务量',dataIndex:'dj',sortable:true,align:'center'},
			{header:'手工业务量',dataIndex:'sg',sortable:true,align:'center'},
			{header:'折算业务量',dataIndex:'zs',sortable:true,align:'center'},
			{header:'当班天数',dataIndex:'ts',sortable:true,align:'center'},
			{header:'日均业务量',dataIndex:'rj',sortable:true,align:'center'},
			{header:'考核周期',dataIndex:'zq',sortable:true,align:'center'}
		];
...
//				stripeRows:true
			...
			tbar:[{
				...
				iconCls:'Folderup',
				handler:function(){
					fileUpload("30102","上传业务量基础数据批量文件");
				}
			},{
				text:'下载批量模版文件',
				iconCls:'Packagedown',
...
//					document.location = "downLoad.html?txCode=30062";//会导致【MessagePush.onPageLoad(userId);】已注册的后台监听失效
					Ext.getDom('downloadIframe').src = "downLoad.html?tx_code=30102";
				}
			}],
