...
public class Excelf1001511111 extends BusParent implements FileUploadListener {
...
//		logger.info("验证未通过用户："+userInvalidJa.toJSONString());
		...
		//打印不存在内容
		for (int j = 0; j < noZwceArr.size(); j++) {
			logger.info("职务层级："+noZwceArr.get(j)+"【用户导入失败】");
			errorBf.append("职务层级："+noZwceArr.get(j)+"【用户导入失败】<br/>");
		}
		for (int j = 0; j < noBmArr.size(); j++) {
			logger.info("机构部门："+noBmArr.get(j)+"【用户导入失败】");
...
//			logger.info("用户初始化,path:"+path);
//			path = path+"userfile\\file\\"+"重庆工行机构部门信息表.xls";
			logger.info("用户初始化文件绝对路径path:"+path);
