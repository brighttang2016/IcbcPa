...
public class Excelf30122 extends BusParent implements FileUploadListener {
	private static String txCode = "30122";
	@Autowired
	MediumBus mediumBus;
	...
	public void doBat(Map retMap,String data){
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			condMap.put("seqNum", json.getString("seqNum"));
			condMap.put("rs", json.getString("rs"));
...
//		sqlStr = "delete t_ntmisc_bzcpmx t where user_id = ? and zq=? and t.used=? and t.seq_num=? ";
		sqlStr = "delete t_ntmisc_zhtsmx t where user_id = ? and zq=? and t.used=? ";
		this.updateBat(condDelList, daoParent, retMap, sqlStr, 1);
		//重新插入当期数据
		sqlStr = "insert into t_ntmisc_zhtsmx(user_id,seq_num,rs,oper_time,used,zq,point) values(?,?,?,?,?,?,?)";
...
//		mpc.pushMessage(tmd.get("userId").toString(), "准备写入excel");
		if(txCode.equals(tmd.get("txCode"))){
			...
			logger.info("特色业务考核基础数据上传成功,正在解析文件");
			//解析excel
			...
			try {
				...
				ArrayList<String> zqList = new ArrayList<String>();//周期数组
...
//					LinkedHashMap<String, Object> rowMap = new LinkedHashMap<String, Object>();
					...
					String seqNum = sheet.getCell(2, i).getContents();//产品序号
					String rs = sheet.getCell(4,i).getContents();//完成值
