...
public class JxBzcpCalcRcv extends BusParent implements BusReceiver{
	@Autowired
	...
	public void jxCalc(JSONArray userYwlJa){
		...
		for (int i = 0; i < userYwlJa.size(); i++) {
			...
			iu.putCondition(condition, "JX_BZCP_WD", userJx+"");
			iu.putCondition(condition, "ZSF_BZCP_WD", fw+"");
			iu.putCondition(condition, "userId", ywlJson.getString("user_id"));
			iu.putCondition(condition, "zq", ywlJson.getString("zq"));
			sqlStr  = "update t_ntmisc_userjx t set t.JX_BZCP_WD=?,t.ZSF_BZCP_WD=? where t.user_id = ? and t.zq=?";
	...
	public void fwCalc(JSONArray userYwlJa){
		...
		System.out.println("标准产品绩效计算（网点考核），计算fwSum后的业务量数据："+userYwlJa.toJSONString());
		logger.info("标准产品绩效计算（网点考核），计算fwSum后的业务量数据："+userYwlJa.toJSONString());
	}
...
//				System.out.println(wSumJsonTemp.toJSONString());
		...
		System.out.println("标准产品绩效计算（网点考核）,计算wSum后的标准产品数据："+userYwlJa.toJSONString());
		logger.info("标准产品绩效计算（网点考核）,计算wSum后的标准产品数据："+userYwlJa.toJSONString());
	}
	...
	public void levelNum(JSONArray ywlJa,TransactionMapData tmd){
		for (int i = 0; i < ywlJa.size(); i++) {
			...
			sqlStr = "select t.rs_kh,t.zb_bzcp from ("
			...
			try {
				...
				ywlJson.put("zb", rsKhZbJson.getString("zb_bzcp"));//所在考核分组标准产品总包(网点考核)
			} catch (Exception e) {
		...
		logger.info("标准产品绩效计算（网点考核），所在考核分组总人数、业务量总包计算结果"+ywlJa.toJSONString());
	}
...
//		JSONArray userYwlJa = ja;// 当前用户可见标准产品原始数据
		...
		this.jxCalc(userYwlJa);//最终标准产品绩效计算
