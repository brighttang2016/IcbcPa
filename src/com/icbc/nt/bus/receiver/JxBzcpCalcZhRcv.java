...
public class JxBzcpCalcZhRcv extends BusParent implements BusReceiver{
	@Autowired
	 ...
	 * @param zbblJa
	 */
	public void jxCalc(JSONArray userYwlJa,JSONArray zbblJa){
		...
		for (int i = 0; i < userYwlJa.size(); i++) {
			...
			float userJx = iu.getFloatDecimal(zb*(fw/fwSum)+"", 2);//分支行考核绩效
			iu.rmCondition(condition);
			iu.putCondition(condition, "JX_BZCP_ZH", userJx+"");
			iu.putCondition(condition, "ZSF_BZCP_ZH", fw+"");
			iu.putCondition(condition, "userId", ywlJson.getString("user_id"));
			iu.putCondition(condition, "zq", ywlJson.getString("zq"));
			sqlStr  = "update t_ntmisc_userjx t set t.JX_BZCP_ZH=?,t.ZSF_BZCP_ZH=? where t.user_id = ? and t.zq=?";
		...
		System.out.println("标准产品绩效计算（网点考核），写入绩效、折算分，userYwlJa："+userYwlJa.toJSONString());
		logger.info("标准产品绩效计算（网点考核），写入绩效、折算分，userYwlJa："+userYwlJa.toJSONString());
	}
...
//			fwSumJson.put("orgid", orgId);
		...
		System.out.println("标准产品绩效计算（支行考核），计算fwSum后的业务量数据："+userYwlJa.toJSONString());
		logger.info("标准产品绩效计算（支行考核），计算fwSum后的业务量数据："+userYwlJa.toJSONString());
	}
...
//				System.out.println(wSumJsonTemp.toJSONString());
		...
		System.out.println("标准产品绩效计算（分支行考核）,计算wSum后的标准产品数据："+userYwlJa.toJSONString());
		logger.info("标准产品绩效计算（分支行考核）,计算wSum后的标准产品数据："+userYwlJa.toJSONString());
	}
	
	/**
	 * 分支行考核，当前用户同一层级考核人员数目、总包设置
	 * @param zbblJa
	 * @param ywlJa
	 */
	public void levelNum(JSONArray ywlJa,TransactionMapData tmd){
		...
		for (int i = 0; i < ywlJa.size(); i++) {
			...
			sqlStr =  " select t.jbjx_pid,t.jbjx_id,t.zq,t.khfw,sum(t.rs_kh) rs_kh,sum(t.zb_bzcp) zb_bzcp from ( " 
...
//			iu.putCondition(condition, "orgid", orgid);
			...
			try {
				...
				ywlJson.put("zrs", rsKhZbJson.getString("rs_kh"));//所在考核分组总人数（分支行考核）
				ywlJson.put("zb", rsKhZbJson.getString("zb_bzcp"));//所在考核分组标准产品总包(分支行考核)
			} catch (Exception e) {
		...
		logger.info("标准产品绩效计算（分支行考核），所在考核分组总人数、业务量总包计算结果"+ywlJa.toJSONString());
	}
...
//		int index = Integer.parseInt(tmd.get("index").toString());
		sqlStr = " select a.user_id,a.point w,a.zq,a.oper_time,a.used,b.name,b.orgid,b.depid, " 
				+  " c.jbjx_pid,c.jbjx_pname,c.jbjx_id,c.jbjx_name " 
				+  " from t_ntmisc_usermova a,t_ntmisc_user b ,t_ntmisc_jobjx c " 
				+  " where  a.user_id = b.userid and b.jbjx_id = c.jbjx_id and a.used='0' and a.zq=? and b.orgid in  " 
				+ orgIn;
		logger.info("执行sql,标准产品绩效计算（网点考核），计算w值,设置w值后,sqlStr:"+sqlStr+"------condition:"+condition);
		this.queryManu(userYwlJa, condition, sqlStr, daoParent, 1);//标准产品基础数据查询
		logger.info("标准产品绩效计算（网点考核），计算w值,设置w值后，userYwlJa："+userYwlJa);
	}
	...
	public void doWork(JSONArray ja, LinkedHashMap<String, Object> condition,
		...
		System.out.println("标准产品绩效计算");
		logger.info("标准产品绩效计算");
		ja.clear();
		/*tmd.put("errorCode", "12");
		tmd.put("errorMsg","标准产品绩效计算中...");
		...
		//查询所有可见标准产品明细数据
		JSONArray userYwlJa = new JSONArray();// 当前用户可见标准产品原始数据
//		JSONArray userYwlJa = ja;// 当前用户可见标准产品原始数据
		...
		this.wSumCalc(userYwlJa);//计算wSum
		this.levelNum(userYwlJa,tmd);//当前用户同绩效考核岗位人数、对应的总包设置
		this.fwCalc(userYwlJa);//计算fw、fwsum
		this.jxCalc(userYwlJa,zbblJa);//最终标准产品绩效计算
