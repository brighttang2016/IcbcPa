...
public class JxDxCalcZhRcv extends BusParent implements BusReceiver{
	@Autowired
	...
	public void jxCalc(JSONArray userYwlJa){
		...
		for (int i = 0; i < userYwlJa.size(); i++) {
			...
			iu.putCondition(condition, "JX_DX_ZH", userJx+"");
			iu.putCondition(condition, "ZSF_DX_ZH", fw+"");
			iu.putCondition(condition, "userId", ywlJson.getString("user_id"));
			iu.putCondition(condition, "zq", ywlJson.getString("zq"));
			sqlStr  = "update t_ntmisc_userjx t set t.JX_DX_ZH=?,t.ZSF_DX_ZH=? where t.user_id = ? and t.zq=?";
		...
		System.out.println("定性绩效计算（支行考核），写入绩效、折算分，userYwlJa："+userYwlJa.toJSONString());
		logger.info("定性绩效计算（支行考核），写入绩效、折算分，userYwlJa："+userYwlJa.toJSONString());
	}
...
//			fwSumJson.put("orgid", orgId);
		...
		System.out.println("定性绩效计算（支行考核），计算fwSum后的业务量数据："+userYwlJa.toJSONString());
		logger.info("定性绩效计算（支行考核），计算fwSum后的业务量数据："+userYwlJa.toJSONString());
	}
...
//			String orgid = ywlJson.getString("orgid");
		...
		*/
			sqlStr =  " select t.jbjx_pid,t.jbjx_id,t.zq,t.khfw,sum(t.rs_kh) rs_kh,sum(t.zb_dx) zb_dx from ( " 
...
//			iu.putCondition(condition, "orgid", orgid);
		...
		logger.info("定性绩效计算（支行考核），所在考核分组总人数、业务量总包计算结果"+ywlJa.toJSONString());
	}
...
//					logger.info("############################:"+userJson.toJSONString());
	...
	public void wCalc(JSONArray userYwlJa,TransactionMapData tmd){
		...
		sqlStr = " select a.user_id,a.point w,a.oper_time,a.used,a.zq,b.orgid,b.depid,c.jbjx_pid,c.jbjx_pname,c.jbjx_id,c.jbjx_name " 
				...
				+  orgIn;
		logger.info("执行sql,定性绩效计算（支行考核），计算w值,设置w值后,sqlStr:"+sqlStr+"------condition:"+condition);
		this.queryManu(userYwlJa, condition, sqlStr, daoParent, 1);
		logger.info("定性绩效计算（支行考核），计算w值,设置w值后，userYwlJa："+userYwlJa);
	}
