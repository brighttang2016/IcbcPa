...
public class JxZhtsCalcZhRcv extends BusParent implements BusReceiver{
	@Autowired
	...
	public void jxCalc(JSONArray userYwlJa){
		...
		for (int i = 0; i < userYwlJa.size(); i++) {
			...
			iu.putCondition(condition, "JX_TSCP_ZH", userJx+"");
			iu.putCondition(condition, "ZSF_TSCP_ZH", fw+"");
			iu.putCondition(condition, "userId", ywlJson.getString("user_id"));
			iu.putCondition(condition, "zq", ywlJson.getString("zq"));
			sqlStr  = "update t_ntmisc_userjx t set t.JX_TSCP_ZH=?,t.ZSF_TSCP_ZH=? where t.user_id = ? and t.zq=?";
		...
		System.out.println("支行特色产品绩效计算（支行考核），写入绩效、折算分，userYwlJa："+userYwlJa.toJSONString());
		logger.info("支行特色产品绩效计算（支行考核），写入绩效、折算分，userYwlJa："+userYwlJa.toJSONString());
	}
...
//			fwSumJson.put("orgid", orgId);
		...
		System.out.println("支行特色产品绩效计算（支行考核），计算fwSum后的业务量数据："+userYwlJa.toJSONString());
		logger.info("支行特色产品绩效计算（支行考核），计算fwSum后的业务量数据："+userYwlJa.toJSONString());
	}
...
//				System.out.println(wSumJsonTemp.toJSONString());
		...
		System.out.println("支行特色产品绩效计算（支行考核）,计算wSum后的标准产品数据："+userYwlJa.toJSONString());
		logger.info("支行特色绩效计算（支行考核）,计算wSum后的标准产品数据："+userYwlJa.toJSONString());
	}
...
//			String orgid = ywlJson.getString("orgid");
			...
			/*sqlStr = "select t.rs_kh,t.zb_zhts from ("
					+ " select a.zq,a.jbjx_pid,a.jbjx_id,b.khfw, a.rs_kh,b.zb_ywl,b.zb_bzcp,b.zb_zhts,b.zb_dx"
					...
					+ " ) t where t.zq=? and t.jbjx_pid=? and t.jbjx_id=? and  t.khfw=?";
		*/
			sqlStr =  " select t.jbjx_pid,t.jbjx_id,t.zq,t.khfw,sum(t.rs_kh) rs_kh,sum(t.zb_zhts) zb_zhts from ( " 
...
//			iu.putCondition(condition, "orgid", orgid);
			...
			try {
				...
				logger.error("|zq"+zq+"jbjx_pid|"+jbjx_pid+"|jbjx_id:"+jbjx_id+",获取用户所在考核分组总人数、总包失败");
				tmd.put("errorCode", "11");
				tmd.put("errorMsg", "获取用户所在考核分组总人数、总包失败");
				tmd.put("finish", "1");
			}
		}
		logger.info("支行特色产品绩效计算（支行考核），所在考核分组总人数、业务量总包计算结果"+ywlJa.toJSONString());
	}
	...
	public void wCalc(JSONArray userJa,TransactionMapData tmd){
		...
		System.out.println("支行特色绩效计算，支行考核，userJa设置w值后："+userJa.toJSONString());
		logger.info("支行特色绩效计算，支行考核，userJa设置w值后："+userJa.toJSONString());
	}
...
//		String orgIn = mediumBus.getOrgIn(orgId);
		...
		this.wCalc(userJa,tmd);//设置特色产品考核w值（与网点考核一样）
